...
app.post("/signup", upload.single("avatar"), async (req, res, cb) => {
    ...
    res.render('noreg.html',{name:"dear user",message:"this number is already registered",bod:"login",linkm:"go to login",title:"Already registered"});
}
});
...
app.post("/logind", async (req, res) => {
	...
	if (await bcrypt.compare(loginpow, userlogin.password)) {
        ...
        res.render('noreg.html',{name:"",message:"Hi! "+loginuser+" You are logged in",bod:"login",linkm:"exit",title:"Welcome"});
        
	}
    else {
        console.log("wrong password");
        res.render('noreg.html',{name:loginuser,message:"your password is incorrect",bod:"login",linkm:"Try again",title:"Wrong password"});
    }
}
else {
    console.log("not registered");
    res.render('noreg.html',{name:loginuser,message:"you are not registered",bod:"regis",linkm:"Go register yourself",title:"Not registered"});
}
});
...
app.post("/forgetotp", async (req, res) => {
    ...
    otp = Math.floor(1000 + Math.random() * 9000);
	console.log(otp);
	 const  response = await fast2sms.sendMessage({authorization:process.env.API_KEY, message : otp, numbers : [req.body.forgetnumber]});
	 console.log(response);
	res.redirect("otpforget.html");
    }
    else {
        res.render('noreg.html',{name:"dear user",message:"you are not registered",bod:"regis",linkm:"Go register yourself",title:"Not registered"});
    }
...
if (otp==req.body.otpinforget) {
    ...
    res.render('noreg.html',{name:" ",message:"wrong otp",bod:"forget",linkm:"try again",title:"invalid otp"});
}
});
app.post("/newpass", async (req, res) => {
  ...
  res.render('noreg.html',{name:"dear"+usernumber.username,message:" your password is updated successfully",bod:"login",linkm:"Go to login",title:"password updated"});
    });
